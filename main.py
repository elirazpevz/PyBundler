...
def main():
    ...
    pyvers = get_option(PYTHON_VERSIONS)
    platforms = get_option(PLATFORMS)

    versions = (get_all_versions(package, pyvers, platforms))
    version_set = list(set(v['version'] for v in versions))
    ...
    selected_version_names = get_option(sorted(version_platforms, reverse=True))
